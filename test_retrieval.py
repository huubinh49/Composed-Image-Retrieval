...
def test(opt, model, testset):
    ...
    for i, query in tqdm(enumerate(all_queries)):
        sims = query.dot(all_imgs.T)

        if test_queries:
            if test_queries[i]['source_img_id'] < len(sims):
                sims[test_queries[i]['source_img_id']] = -10e10
        nn_result = np.argsort(-sims)[:110]
        nn_result = [all_captions[nn] for nn in nn_result]
        for it, k in enumerate([1, 5, 10, 50, 100]):
            if all_target_captions[i] in nn_result[:k]:
                rec[it] += 1

    for it, k in enumerate([1, 5, 10, 50, 100]):
        rec[it] /= len(all_queries)
        out += [('recall_top' + str(k) + '_correct_composition', rec[it])]
    return out